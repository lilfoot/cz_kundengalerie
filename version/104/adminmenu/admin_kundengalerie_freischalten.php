<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['cAction']) && $_POST['cAction'] == 'kundengalerie_freischalten')
	{
		...
		$oUpdate->bAktiv = "y";
		$oUpdate->cKundenkommentar = $oKundengalerie->db()->escape($_POST['cKundenkommentar']);
		...
		if ($bOk)
		{
			...
			if ($_POST['selKupon'] == 1)
			{
				...
				if (is_object($oKupon))
				{
					...
					$oKundengalerie->setzeKuponAnzahl($_POST['kKunde']);
				}
			}
			
			if ($_POST['selGuthaben'] == 1)
			{
				//var_dump($_POST);
				$bOk = $oKundengalerie->bucheGuthaben($_POST['kKunde'], str_replace(',', '.', $_POST['fGuthabenWert']));
				
				if ($bOk)
				{
					$oObj = new stdClass();
					$oObj->tkunde = new Kunde($_POST['kKunde']);
					$oObj->fWert = str_replace(',', '.', $_POST['fGuthabenWert']);
					$oObj->fGuthabenKonto = $oKundengalerie->gibKundenguthaben($_POST['kKunde']);
